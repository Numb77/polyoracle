...
import type { ActivePosition, AgentVote, TradeResolved } from "@/lib/types";
...
function useWindowCountdown(windowTs: number) {
	...
	const calc = () =>
		Math.max(0, Math.floor((windowCloseTs - Date.now()) / 1000));
...
function AgentRow({ vote }: { vote: AgentVote }) {
	...
	return (
		...
		>
			...
			<span className={cn("text-xs font-bold w-14 text-center", dirColor)}>
				{vote.vote}
			</span>
...
function TradeDetailModal({
	...
	const holdMs = trade.opened_at
		? trade.opened_at - trade.window_ts * 1000
		: null;
	const holdSec =
		holdMs != null && holdMs >= 0 ? Math.floor(holdMs / 1000) : null;
	...
	return (
		...
		>
			<div
				className="w-full max-w-md mx-4 rounded-xl border bg-zinc-950 shadow-2xl overflow-hidden"
				style={{ borderColor: "var(--border-color)" }}
			>
				{/* Header */}
				<div
				...
				>
					<div className="flex items-center gap-2">
						...
						<span
							className={cn(
								"font-bold text-sm",
								trade.won ? "text-accent-green" : "text-accent-red",
							)}
						>
							{trade.won ? "WIN ✓" : "LOSS ✗"}
						</span>
					...
					<div className="grid grid-cols-2 gap-2 text-xs font-mono">
						<div className="bg-zinc-900 rounded p-2">
							...
							<div className="text-white truncate">
								{marketLabel(trade.market, asset)}
							</div>
						</div>
						<div className="bg-zinc-900 rounded p-2">
						...
						{trade.window_delta_pct != null && (
							<div className="bg-zinc-900 rounded p-2">
								...
								<div
									className={cn(
										"font-bold",
										trade.window_delta_pct >= 0
											? "text-accent-green"
											: "text-accent-red",
									)}
								>
									{trade.window_delta_pct >= 0 ? "+" : ""}
									{trade.window_delta_pct.toFixed(3)}%
								</div>
							</div>
						)}
						{holdSec != null && (
					...
					{trade.agent_votes && trade.agent_votes.length > 0 && (
						<div>
							...
							<div
								...
								style={{ borderColor: "var(--border-color)" }}
							>
								{trade.agent_votes.map((v) => (
									<AgentRow key={v.agent} vote={v} />
								))}
							</div>
						</div>
					)}
		...
		>
			...
			<div className="flex-1 min-w-0">
				...
				<div className="text-[10px] text-text-secondary mt-0.5">
					@ {pos.price.toFixed(3)} · ${pos.size_usd.toFixed(2)} · conf{" "}
					{pos.confidence.toFixed(0)}
				</div>
			</div>
...
export function TradeHistory() {
	...
	// activePositions already includes all assets (merged in useBotState withAsset)
	const activePositions = state.activePositions;

	const { lastClaimsRecovery } = state;

	const [selectedTrade, setSelectedTrade] = useState<TradeResolved | null>(
		null,
	);
